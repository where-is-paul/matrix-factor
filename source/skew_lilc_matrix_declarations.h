// -*- mode: c++ -*-
#ifndef _SKEW_LILC_MATRIX_DECLARATIONS_H_
#define _SKEW_LILC_MATRIX_DECLARATIONS_H_
...
class skew_lilc_matrix : public lil_sparse_matrix<el_type>
{
public:
...
public:
	...
	*/
	skew_lilc_matrix (int n_rows = 0, int n_cols = 0) : lil_sparse_matrix<el_type> (n_rows, n_cols) 
	{
		m_x.reserve(n_cols);
	...
	*/
	...
	/*! \brief Performs an LDL' factorization of this matrix. 
		...
		The pivoted matrix P'AP will be stored in place of A. In addition, the L and D factors of P'AP will be stored in L and D (so that P'AP = LDL').
		Use the partial pivoting strategy.
	...
	*/
	void ildlpp(lilc_matrix<el_type>& L, skew_block_diag_matrix<el_type>& D, idx_vector_type& perm, const double& fill_factor, const double& tol);
	
	/*! \brief Performs an LDL' factorization of this matrix. 
		
		The pivoted matrix P'AP will be stored in place of A. In addition, the L and D factors of P'AP will be stored in L and D (so that P'AP = LDL').
		Use the modified partial pivoting strategy.
		\param L the L factor of this matrix.
		\param D the D factor of this matrix.
		\param perm the current permutation of A.
		\param fill_factor a parameter to control memory usage. Each column is guaranteed to have fewer than fill_factor*(nnz(A)/n_col(A)) elements.
		\param tol a parameter to control agressiveness of dropping. In each column, elements less than tol*norm(column) are dropped.
	*/
	void ildlmpp(lilc_matrix<el_type>& L, skew_block_diag_matrix<el_type>& D, idx_vector_type& perm, const double& fill_factor, const double& tol);
	
	/*! \brief Performs an LDL' factorization of this matrix. 
		
		The pivoted matrix P'AP will be stored in place of A. In addition, the L and D factors of P'AP will be stored in L and D (so that P'AP = LDL').
		Use the rook pivoting strategy.
		\param L the L factor of this matrix.
		\param D the D factor of this matrix.
		\param perm the current permutation of A.
		\param fill_factor a parameter to control memory usage. Each column is guaranteed to have fewer than fill_factor*(nnz(A)/n_col(A)) elements.
		\param tol a parameter to control agressiveness of dropping. In each column, elements less than tol*norm(column) are dropped.
	*/
	void ildlrp(lilc_matrix<el_type>& L, skew_block_diag_matrix<el_type>& D, idx_vector_type& perm, const double& fill_factor, const double& tol);
	...
	*/
	inline void pivot(skew_swap_struct<el_type> s, vector<bool>& in_set, lilc_matrix<el_type>& L, const int& k, const int& r);
	...
	*/
	...
	{
		int offset = first[j];
		if ((offset >= (int)con.size()) || con.empty() || con[offset] == k)
			return;
...
#include "skew_lilc_matrix_find_level_set.h"
#include "skew_lilc_matrix_find_root.h"
#include "skew_lilc_matrix_sym_rcm.h"
#include "skew_lilc_matrix_sym_perm.h"
#include "skew_lilc_matrix_sym_equil.h"
#include "skew_lilc_matrix_ildlpp.h"
#include "skew_lilc_matrix_ildlmpp.h"
#include "skew_lilc_matrix_ildlrp.h"
#include "lilc_matrix_ildl_helpers.h"
#include "skew_lilc_matrix_ildl_helpers.h"
#include "skew_lilc_matrix_pivot.h"
#include "skew_lilc_matrix_load.h"
#include "skew_lilc_matrix_save.h"
#include "skew_lilc_matrix_to_string.h"
