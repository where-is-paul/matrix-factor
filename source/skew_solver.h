#ifndef _SKEW_SOLVER_H
#define _SKEW_SOLVER_H

#include <iostream>
#include <string.h>
#include "lilc_matrix.h"
#include "skew_lilc_matrix.h"
//#include <sys/time.h>
...
bool skew_save_perm(const std::vector<el_type>& vec, std::string filename)
{
	...
	public:
		...
		*/
		void solve(double fill_factor, double tol)
		{
			...
			//A.sym_equil(); cout << A << endl; cout << A.S << endl;
			//A.sym_rcm(perm); cout << A << endl; cout << perm << endl;
			for (int i = 0; i < A.n_cols(); i++) perm.push_back(i);
			A.sym_perm(perm); cout << A << endl;
		...
		*/
		void save()
		{
			...
			skew_save_perm(perm, "output_matrices/outP.mtx");
