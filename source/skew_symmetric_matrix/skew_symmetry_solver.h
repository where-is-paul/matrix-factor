#ifndef SKEW_SYMMETRY_SOLVER_H
#define SKEW_SYMMETRY_SOLVER_H

#include <iostream>
#include <iomanip>
#include <string>
#include <cstring>
#include "skew_symmetry_matrix.h"
...
{
	public:
		skew_symmetry_matrix<el_type> A;	///<The matrix to be factored.
		...
		*/
		void solve_pp(double fill_factor, double tol)
		{
		...
		void solve_mpp(double fill_factor, double tol)
		{
			perm.reserve(A.n_cols());
			//for (int i = 0; i < A.n_cols(); i++) {
			//	perm.push_back(i);
		...
		void solve_rp(double fill_factor, double tol)
		{
			perm.reserve(A.n_cols());
			//for (int i = 0; i < A.n_cols(); i++) {
			//	perm.push_back(i);
		...
		*/
		bool save_perm(const vector<int>& vec, std::string filename)
		{
