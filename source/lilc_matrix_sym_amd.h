...
namespace amd {
	...
	inline int amd_unflip(const int& i) { return i<0 ? amd_flip(i) : i; }
...
inline void lilc_matrix<el_type> :: sym_amd(vector<int>& perm) {
	...
	int* W       = new int[9*(n+1)]; /* get workspace */
	int* len     = W;
	int* nv      = W +   (n+1);
	int* next    = W + 2*(n+1);
	int* head    = W + 3*(n+1);
	...
	mark = amd::cs_wclear(0, 0, w, n);         /* clear w */
	...
	while (nel < n)                         /* while (selecting pivots) do */
	{
		...
		mark = amd::cs_wclear(mark, lemax, w, n);  /* clear w if necessary */
		...
		mark = amd::cs_wclear(mark+lemax, lemax, w, n);    /* clear w */
	...
	{
		if(Cp[i] == -1) k = amd::cs_tdfs(i, k, head, next, temp_perm, w);
	}
