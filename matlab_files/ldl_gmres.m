function [x,flag,relres,iter,resvec] = ldl_gmres(A,b,restart,tol,maxit,M1,M2,M3,x,varargin)
...
if ((nargin >= 8) && ~isempty(M3))
    existM3 = 1;
    [m3type,m3fun,m3fcnstr] = iterchk(M3);
    if strcmp(m3type,'matrix')
        if ~isequal(size(M3),[m,m])
            error(message('MATLAB:gmres:PreConditioner3Size', m));
        end
    end
else
    existM3 = 0;
...
if existM3
    r = iterapp('mldivide',m3fun,m3type,m3fcnstr,r,varargin{:});
    if ~x0iszero
        minv_b = iterapp('mldivide',m3fun,m3type,m3fcnstr,minv_b,varargin{:});
    else
        minv_b = r;
    end
    if ~all(isfinite(r)) || ~all(isfinite(minv_b))
...
for outiter = 1 : outer
    ...
    for initer = 1 : inner
		...
		if existM3
            v = iterapp('mldivide',m3fun,m3type,m3fcnstr,v,varargin{:});
            if ~all(isfinite(v))
                flag = 2;
                break
            end
        ...
        if (normr <= tolb || stag >= maxstagsteps || moresteps)
			...
			if existM3
                minv_r = iterapp('mldivide',m3fun,m3type,m3fcnstr,minv_r,varargin{:});
                if ~all(isfinite(minv_r))
                    flag = 2;
                    break
                end
